...
if INSTALL_METHOD == "exe":
    temp = os.path.dirname(
        unicode(sys.executable, sys.getfilesystemencoding()))  # should be exe path to \fpdbroot\pyfpdb
    ...
    FPDB_ROOT_PATH = os.path.dirname(
        unicode(sys.path[0], sys.getfilesystemencoding()))  # should be source path to /fpdbroot
        ...
        for location_node in node.getElementsByTagName('location'):
            ...
            if hud_seat != "":
                ...
                self.location[int(hud_seat)] = (
                    int(location_node.getAttribute('x')), int(location_node.getAttribute('y')))
...
class Database:
    def __init__(self, node):
        ...
        log.debug(
            "Database db_name:'%(name)s'  db_server:'%(server)s'  db_ip:'%(ip)s'  db_user:'%(user)s'  db_pass (not logged)  selected:'%(sel)s'" \
            % {'name': self.db_name, 'server': self.db_server, 'ip': self.db_ip, 'user': self.db_user,
...
class Import:
    ...
    def __str__(self):
        return "    interval = %s\n    callFpdbHud = %s\n    saveActions = %s\n   cacheSessions = %s\n    publicDB = %s\n    sessionTimeout = %s\n    fastStoreHudCache = %s\n    ResultsDirectory = %s" \
               % (
            self.interval, self.callFpdbHud, self.saveActions, self.cacheSessions, self.publicDB, self.sessionTimeout,
            self.fastStoreHudCache, self.ResultsDirectory)
...
class HudUI:
    def __init__(self, node):
        ...
        if node.hasAttribute('aggregation_level_multiplier'): self.agg_bb_mult = node.getAttribute(
        ...
        if node.hasAttribute('seats_cust_nums_high'): self.seats_cust_nums_high = node.getAttribute(
        ...
        if node.hasAttribute('hero_aggregation_level_multiplier'): self.h_agg_bb_mult = node.getAttribute(
        ...
        if node.hasAttribute('hero_seats_cust_nums_low'): self.h_seats_cust_nums_low = node.getAttribute(
            'hero_seats_cust_nums_low')
        if node.hasAttribute('hero_seats_cust_nums_high'): self.h_seats_cust_nums_high = node.getAttribute(
            'hero_seats_cust_nums_high')
...
class Config:
    ...
    def get_db_parameters(self):
        ...
        try:
            ...
            print "AttributeError for {0} ({1})".format(name, attError.message)
    ...
    def get_hud_ui_parameters(self):
        ...
        try:
            hui['deck_type'] = unicode(self.ui.deck_type)
        except:
            hui['deck_type'] = u'colour'

        try:
            hui['card_back'] = unicode(self.ui.card_back)
        except:
            hui['card_back'] = u'back04'
